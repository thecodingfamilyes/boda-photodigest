...
export default class Instagram {
			...
			return res.text().then(function(response) {
				...
				if (json.status == 'ok') {
					...
					if (json.followed_by.page_info.has_next_page) {
						...
						}, (reject) => console.log('Error handle response from instagram server(get followers request)'))
					} else {
  ...
  */
	getCsrfToken()
	{
		...
		}).catch(() => console.log('Failed to get instagram csrf token'))
	}
  ...
  */
	auth(username, password)
	{
		...
		}).catch(() => console.log('Instagram authentication failed'))
	}
  ...
  */
	follow(userId, isUnfollow)
	{
		...
		return fetch('https://www.instagram.com/web/friendships/' + userId + (isUnfollow == 1
			? '/unfollow'
			: '/follow'), {
			'method': 'post',
			'headers': headers
  ...
  */
	getFeed(items, cursor)
	{
		items = items
			? items
			: 10;
		return fetch('https://www.instagram.com/graphql/query/?query_id=17866917712078875&fetch_media_item_count=' + items + '&fetch_media_item_cursor=' + cursor + '&fetch_comment_count=4&fetch_like=10', {headers: this.getHeaders()}).then(t =>
		// console.log(t)
		t.json().then(r => r))
	}
  ...
  */
	...
	{
		return fetch('https://api.instagram.com/oembed/?url=' + url, {'headers': this.getHeaders()}).then(t => t.json().then(r => r))
	}
  ...
  */
	searchBy(searchBy, q, cursor, mediaCounter)
	{
		...
		if (searchBy == 'hashtag' && !cursor) {
			return fetch('https://www.instagram.com/explore/tags/' + q + '/', {headers: this.getHeaders()}).then(t => t.text().then(r => JSON.parse(r.match(/\<script type=\"text\/javascript\">window\._sharedData \=(.*)\;<\//)[1])))
		}
